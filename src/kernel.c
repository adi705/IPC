...
int readdir(unsigned char *buf) {
	...
	int OS_SIZE_LOC = 2;	// within bootblock 
    int check;
	// internel_buf is now the bootblock sector
	check = scsi_read(0,1, (char*)internal_buf);
	
    os_size = internal_buf[2];
	check = scsi_read(os_size + 1,1, (char*)internal_buf);
	// copies the block into user provided buffer
	bcopy((char*)internal_buf, (char*)buf, SECTOR_SIZE);
...
int loadproc(int location, int size) {
    // size in bytes is required for allocating memory
	int byte_size = (size * SECTOR_SIZE) + STACK_SIZE;
	uint32_t mem = alloc_memory((uint32_t)byte_size);//returns a memory address
	scsi_read(location,size,(char*)mem);
	create_process(mem, byte_size); 
	return 0;
}
