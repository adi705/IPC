...
struct uhci_queue_head {
	...
	struct uhci_queue_head
		*prev_qh;  /* QH link pointer pointing to this transfer QH */
	char is_top_queue; /* 1 for interrupt, control, bulk transfer queue
			      heads */
...
struct uhci_transfer_unit {
	struct list
		list;    /* Transfer units are link together on the free list */
...
#define UHCI_LP_VF                                                             \
	(1 << 2) /* Switch depth (1)/breadth first, valid only for TDs in      \
		    queues */
#define UHCI_LP_QH                                                             \
	(1 << 1) /* Related Link Pointer points to either Queue Head (1) or    \
		    Transfer Descriptor */
