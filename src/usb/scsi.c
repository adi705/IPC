...
static int scsi_read_write(int dir, int block_start, int block_count,
			   char *data);
...
int scsi_init(struct scsi_ifc *ifc)
{
	...
	DEBUG("Device block size %d, block count %d", scsi->block_size,
	      scsi->total_block_count);
...
static int scsi_test_unit_ready(void)
{
	...
	if (rc == SCSI_RC_FAILED) {
		...
		/* Send REQUEST SENSE command. It should contain info about what
		 * went wrong in previous command, and, more importantly, if we
		 * have a UNIT ATTENTION condition (e.g. media change or power
		 * on reset), this condition will be cleared by the device
		 * (meaning the next retry might succeed).
		 */
		scsi_request_sense();
		return -1;
	}
