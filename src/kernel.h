...
enum
{
	 ...
	 * registers. The numbers are indexes into the global
	 * descriptor table. Shiftlefted since the contents of the
	 * register is more than just the index. (PMSA p. 80).
	 */
	 ...
	 * The Undocumented PC p.978-979 (Port 40h).  Alternate values
	 * are for instance 1193 (1000 Hz) and 119 (10 000 Hz). The
	 * last value will be too have the timer interrupt fire too
	 ...
	 * this when adding to or removing elements from the
	 * start_addr array.
	 */
	NUM_THREADS = 5,
...
struct pcb {
	...
	/*
	 * When d_c == 0, we can turn on interrupts. See
	...
	/*
	 * The base physical address of the process this is just for
	 * project 4
	 */
	uint32_t base;
	/*
	 * Size of memory allocated for this process (code + data +
	 * user stack)
	 */
	uint32_t limit;
	/*
	...
	/* These are used when job is in the ready queue */
	struct pcb *next;
	struct pcb *previous;
};
...
/*
 * Structure describing the contents of an interrupt gate
 * entry. (Protected Mode Software Architecture p. 206)
 */
struct gate_t {
	uint16_t offset_low;
	uint16_t selector;
...
/*
 * Structure describing the contents of the idt and gdt
 * registers. Used for loading the idt and gdt registers. (Protected
 * Mode Software architecture p.42)
 */
struct point_t {
	uint16_t limit;
	uint32_t base;
...
/* Exception handlers don't return anythings */
